...
//            Snackbar.make(startFOB/*any widget to find root ViewGroup*/, "START!!!", Snackbar.LENGTH_SHORT).show()
            findNavController().navigate(R.id.action_login2main,
                bundleOf("userEmail" to "local"))    // goto main game UI with local data use
        }
        ...
        signInButton.setOnClickListener { _ ->
            ...
            when {  // 3 invalid conditions to check for first before 'else' acceptance
                emailStr.isEmpty() ->
                    Snackbar
                            .make(email, getString(R.string.email_required),
                                  Snackbar.LENGTH_LONG)
                            .show()
                !emailRegex.matcher(emailStr).find() ->
                    Snackbar
                        .make(email, getString(R.string.invalid_email),
                              Snackbar.LENGTH_LONG)
                        .show()
...
//                            .show()
                    findNavController().navigate(R.id.action_login2main,
                        bundleOf("userEmail" to emailStr))  // goto main game UI with passed email data
                }
            }
        }
        // Sign up for a new email. Might add database services later.
